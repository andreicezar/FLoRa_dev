...
Scans --json-dir like Scenario 02:
  looks for files whose names contain ["scenario-03","baseline", f"sf{SF}"]
and assembles a bundle per SF with:
  parameters, scalars, (histograms|statistics), (app_)vectors
Also supports a single combined JSON per SF (auto-detects inner lists).

Scoreboard columns:
  Sent, Expected, Sent/Expected (%), Received, PDR (%)
  Total ToA (s), Utilization (%), Efficiency (pkts/s ch)
  Theor/pkt (ms), Scale vs SF7
  Goodput (kbps), GW RxOK (%), Collisions, Avg RSSI (dBm), Avg SNIR (dB)
Plus a small trend check block.
"""
...
# ------------------------------- scanning functions ----------------------------
def name_matches_sf(fname: str, sf: int) -> bool:
    s = fname.lower()
    return ("scenario-03" in s and "baseline" in s and re.search(rf"\bsf0?{sf}\b", s) is not None)

def find_bundle_s03(json_dir: Path, sf: int) -> Dict[str, Path]:
    """
    Scenario-02 style scan in a flat folder, but:
...
def discover_sf_bundles(json_dir: Path, sfs: List[int]) -> List[Tuple[int, Dict[str, Path]]]:
    out: List[Tuple[int, Dict[str, Path]]] = []
    for sf in sfs:
        b = find_bundle_s03(json_dir, sf)
        if b.get("parameters") and b.get("scalars"):
            out.append((sf, b))
    return out

# ------------------------------- radio/ToA helpers (CORRECT FORMULA) -----------------------------------
def parse_bw_hz(s):
    if s is None: return None
    if isinstance(s,(int,float)): return float(s)
...
def lora_toa_seconds(payload_bytes: int, sf: int, bw_hz: float, cr: int,
    ...
    """
    CORRECT Semtech LoRa Time-on-Air formula
    """
...
def payload_bytes_estimate(params, histograms, scalars=None) -> Optional[int]:
    """
    Estimate typical payload bytes.
    Priority:
      1) Histogram/statistics of packet lengths if available.
      2) Scalars: per-node incomingPacketLengths:sum / sentPackets.
...
def pick_total_airtime_seconds(scalars: List[dict], histograms: List[dict]) -> Optional[float]:
    # try histogram/statistics sums first
...
def extract_avg_sf_tp_from_vectors(vectors) -> Tuple[Optional[float], Optional[float]]:
    ...
    for v in vectors:
        ...
        if name == "SF Vector":
            ...
            if values:
                try:
                    avg_sf = sum(float(x) for x in values) / len(values)
                    print(f"DEBUG: Extracted avg SF from vector: {avg_sf:.1f}")  # Remove after testing
                except:
                    pass
        elif name == "TP Vector":
            values = v.get("values", [])
            if values:
                try:
                    avg_tp = sum(float(x) for x in values) / len(values)
                    print(f"DEBUG: Extracted avg TP from vector: {avg_tp:.1f}")  # Remove after testing
                except:
                    pass
...
def analyze_one(bundle: Dict[str, Path], label: str) -> Dict[str, Any]:
    ...
    _vectors   = get_vectors_list(vectors_obj)  # not strictly needed for scoreboard
    ...
    sent_ratio = 100.0*sent/expected if expected else None
    pdr = 100.0*recv/sent if (recv is not None and sent>0) else None

    # Theoretical airtime calculation (FLoRa has no measured airtime)
    ...
    eff  = (recv / toa_s) if (recv is not None and isinstance(toa_s,(int,float)) and toa_s>0) else None

    # Calculate theoretical per-packet time
    if theor_ms is None:
        pl = payload_bytes_estimate(params, histograms, scalars)
        if pl is not None:
            sf = estimate_sf(params, scalars)
    ...
    goodput = None
    pl = payload_bytes_estimate(params, histograms, scalars)
    if pl is not None and recv is not None and sim_s:
        goodput = (pl*8.0*recv)/(sim_s*1000.0)

    rx_started = pick_scalar(scalars, SCALAR_RX["gw_rx_started"])
    rx_ok      = pick_scalar(scalars, SCALAR_RX["gw_rx_ok"])
    rx_ok_pct  = 100.0*rx_ok/rx_started if (isinstance(rx_started,(int,float)) and rx_started>0 and isinstance(rx_ok,(int,float))) else None
    collisions = sum_collisions(scalars)

    # RF averages (weighted) - EXACT field names used
    rssi_mean = None
    for h in histograms:
    ...
    vectors = get_vectors_list(vectors_obj)  # Change _vectors to vectors
    # Extract average SF and TP from vectors, fallback to initial values
    avg_sf, avg_tp = extract_avg_sf_tp_from_vectors(vectors)
    if avg_sf is None:
        avg_sf = estimate_sf(params, scalars)
    if avg_tp is None:
        # Try to get from init data first, then fallback to parameter extraction
...
# ------------------------------- printing --------------------------------------------
def fmt(x, d=2): return f"{x:.{d}f}" if isinstance(x,(int,float)) else "NA"
...
def print_scoreboard(rows):
    # Simplified scoreboard with requested 9 columns only
    columns = [
    ...
    print("\n" + "="*110)
    print("SCENARIO 03 — Spreading Factor Impact (FLoRa) — SCOREBOARD") 
    print("="*110)
...
def trend_checks(rows):
    rows = sorted([r for r in rows if isinstance(r.get("Avg SF"),(int,float))], key=lambda r: r["Avg SF"])
    if not rows: return
    def seq(k): return [r.get(k) for r in rows]
    def mono_up(xs):
        xs=[x for x in xs if isinstance(x,(int,float))]
        return all(xs[i]>=xs[i-1]-1e-9 for i in range(1,len(xs))) if xs else None
    def mono_dn(xs):
        xs=[x for x in xs if isinstance(x,(int,float))]
        return all(xs[i]<=xs[i-1]+1e-9 for i in range(1,len(xs))) if xs else None
    
    pdr_seq = seq("PDR (%)")
    pdr_decreasing = mono_dn(pdr_seq)
    pdr_increasing = mono_up(pdr_seq)
    
    util_ok = mono_up(seq("Utilization (%)"))
    
    print("\n--- Trend checks ---")
    if pdr_decreasing:
        print("PDR decreases with SF:          OK (interference-limited)")
    elif pdr_increasing:
        print("PDR increases with SF:          OK (range-limited scenario)")
    elif any(isinstance(x,(int,float)) for x in pdr_seq):
        print("PDR trend mixed:                CHECK (may indicate transition zone)")
    else:
        print("PDR trend:                      insufficient data")
        
    print(f"Utilization increases with SF:  {'OK' if util_ok else 'CHECK'}" if util_ok is not None else "Util trend: insufficient data")
...
# ------------------------------- main -------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="Analyze FLoRa Scenario 03 (SF Impact) — scans like Scenario 02")
    ap.add_argument("--json-dir", type=Path, default=DEFAULT_JSON_DIR,
                    help="Folder with scenario-03-baseline-sf*.json (combined or split files)")
    ...
    bundles = discover_sf_bundles(args.json_dir, args.sfs)
    if not bundles:
        print(f"[ERR] No SF bundles found in {args.json_dir}. Expected names like 'scenario-03-baseline-sf7.json'."); sys.exit(3)
