...
# - Supports --area filtering like scenario 08
...
# =============================================================================
# Area filtering helpers (copied from scenario 08)
# =============================================================================
...
# =============================================================================
def find_bundle(json_dir: Path, base_tokens: List[str], area: str | None = None) -> Dict[str, Path]:
...
def find_all_bundles_s01(json_dir: Path, area: str | None = None) -> List[Tuple[str, Dict[str, Path]]]:
    """
    ...
    for base in sorted(bases):
        ...
        bundle = find_bundle(json_dir, [t for t in toks if t], area)  # pass area parameter
...
def print_context_table(rows: List[Dict[str, Any]]) -> None:
    ...
    headers = ("Configuration","Nodes","SimTime_s","Interval_s","ADR Enabled","Initial SF","Initial TP (dBm)")
    data_rows = []
    for r in rows:
        row = (
            r.get("Configuration",""),
    ...
    for i, width in enumerate(widths):
        if i == 0:  # Left-align first column (Configuration)
    ...
    total_width = sum(widths) + 3 * (len(widths) - 1)  # 3 chars per separator: " | "
...
def main():
    ...
    bundles = find_all_bundles_s01(args.json_dir, args.area)
    if not bundles:
